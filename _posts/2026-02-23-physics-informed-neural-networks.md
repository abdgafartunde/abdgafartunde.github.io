...
You parameterise the unknown $u$ with a neural network $u_\theta(x)$. Using automatic differentiation, you compute whatever partial derivatives $\mathcal{N}$ requires — directly through the computational graph of the network. No discretization of the differential operator, no stiffness matrices, no mesh.

The training loss is typically a weighted sum:

$$\mathcal{L}(\theta) = \lambda_r \frac{1}{N_r} \sum_{i=1}^{N_r} |\mathcal{N}[u_\theta](x_i^r)|^2 + \lambda_b \frac{1}{N_b} \sum_{j=1}^{N_b} |\mathcal{B}[u_\theta](x_j^b)|^2 + \lambda_d \frac{1}{N_d} \sum_{k=1}^{N_d} |u_\theta(x_k^d) - u_k^{\text{obs}}|^2.$$

The first term measures how well the PDE is satisfied at collocation points scattered through the domain. The second enforces boundary conditions. The third, present only when you have observational data, fits the network to measurements.
...
The same idea extends to initial conditions in time-dependent problems. If $u(x,0) = g(x)$, write $u_\theta(x,t) = g(x) + t \cdot v_\theta(x,t)$. At $t=0$, the initial condition is exactly satisfied.
