...
**3D reconstruction from 2D survey lines.** Most ERT surveys are conducted along 1D or 2D transects and interpreted as 2D cross-sections. True 3D aquifer geometry requires 3D inversion of areal survey data — computationally more demanding but geometrically more informative. Scalable 3D ERT inversion algorithms are an active research area.
